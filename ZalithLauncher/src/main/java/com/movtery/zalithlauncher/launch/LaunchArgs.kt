...
class LaunchArgs(
    ...
    private fun getLauncherJvmArgs(lwjglComponent: String): List<String> {
        ...
        // Single owner of java.library.path
    ...
    private fun getVersionJvmArgs(lwjglComponent: String): Array<String> {
        ...
        resolvedVersionInfo.arguments?.jvm?.forEach { arg ->
            processJvmArg(arg)?.let(jvmArgs::add)
        }

        return JSONUtils.insertJSONValueList(jvmArgs.toTypedArray(), varArgMap)
    }

    private fun processJvmArg(arg: Any): String? = (arg as? String)?.let { argument ->
        when {
            argument == "\${classpath}" -> null
            argument.startsWith("-cp") -> null

            // LaunchArgs already owns this property.
            argument.startsWith("-Djava.library.path=") -> null

            argument.startsWith("-Dorg.lwjgl.librarypath=") ||
                    argument.startsWith("-Djna.boot.library.path=") -> null

            argument.startsWith("-DignoreList=") -> "$argument,$versionFileName.jar"
